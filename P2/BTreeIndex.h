...
class BTreeIndex {
 public:
  ...
  RC locate_rec(int searchKey, IndexCursor& cursor, int level, PageId pid);
  ...
  RC insert_rec(int key, const RecordId& rid, 
              PageId node_pid, int level, 
              PageId& sibling_pid, int& sibling_key);

  void print();
  void print_rec(PageId pid, int level, std::string space);
